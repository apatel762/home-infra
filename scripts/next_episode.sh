...
#       $3 (optional): Set to any value to not append a new line to the text.
# OUTS: None
...
# DESC: Find the current episode using the marker file (which is just a number)
# ARGS: None
...
# DESC: Open the next episode using the default software
# ARGS: $1 (required): The current marker file name
# OUTS: None
function open_next_episode() {
    local next_ep_filename=$(ls | grep $1 | sort | head -n1 )
    if [[ -z $next_ep_filename ]]; then
        pretty_print "Could not find episode $1!" $fg_red
        script_exit "Exiting..." 1
    fi
    xdg-open "$next_ep_filename" \
        || script_exit "xdg-open failed" 1
}
