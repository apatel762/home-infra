...
function ensure_toolbox_terminal() {
    if [ -f /run/.containerenv ] && [ -f /run/.toolboxenv ]; then
        :
    else
        script_exit "You must run this script from a toolbox. Please enter a toolbox container and try again." 1
    fi
}
...
function enter_python_venv() {
    if [ -f "$ROOTDIR/../venv/bin/activate" ]; then
        ...
        script_exit "Could not find '$ROOTDIR/../venv/bin/activate'. Are you running this script via the Makefile?" 1
    fi
}
...
function await_ostree_idle() {
    if [ -f scripts/await-ostree-idle.sh ]; then
...
function main() {
    ...
    ensure_toolbox_terminal
    enter_python_venv

    await_ostree_idle

    pretty_print "=======================================================================
Applying configuration via Ansible for:
$(ansible all --inventory-file hosts.ini --list-hosts)
=======================================================================" "${fg_white-}"
